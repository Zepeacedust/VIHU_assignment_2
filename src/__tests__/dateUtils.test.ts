...
import { add, getCurrentYear, isDateBefore, isWithinRange, isSameDay } from  '../dateUtils';
...
describe("Date Utils", () => {
  ...
  describe("Add", ()=> {
    test('12 months in a year', () => {
      ...
      // 
      expect(add(now, 12, DATE_UNIT_TYPES.MONTHS).getTime()).to.equal(add(now, 1, DATE_UNIT_TYPES.YEARS).getTime());
    });
    
    test('360 seconds in 60 minutes', () => {
      const now = moment().toDate()
      // 
      expect(add(now, 3600, DATE_UNIT_TYPES.SECONDS).getTime()).to.equal(add(now, 60, DATE_UNIT_TYPES.MINUTES).getTime());
    });
  })
